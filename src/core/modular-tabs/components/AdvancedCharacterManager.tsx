...
import type { Character, Project } from '../../../client/src/lib/types';
import type { ModularTabConfig } from '../types/TabConfig';

// Import all the existing Character Manager components to preserve functionality
import { CharacterDetailView } from '../../../client/src/components/character/CharacterDetailView';
import { CharacterPortraitModal } from '../../../client/src/components/character/CharacterPortraitModalImproved';
import { CharacterGenerationModal, type CharacterGenerationOptions } from '../../../client/src/components/character/CharacterGenerationModal';
import { CharacterTemplates } from '../../../client/src/components/character/CharacterTemplates';
import { CharacterCreationLaunch } from '../../../client/src/components/character/CharacterCreationLaunch';
import { CharacterDocumentUpload } from '../../../client/src/components/character/CharacterDocumentUpload';
import { CharacterCreationService } from '../../../client/src/lib/services/characterCreationService';

interface AdvancedCharacterManagerProps {
  projectId: string;
...
/**
 * AdvancedCharacterManager - Exact replica of CharacterManager functionality
 * but configurable through ModularTabConfig
 * 
 * Preserves ALL features:
 * - AI generation and templates
 * - Portrait generation and upload
 * - Bulk operations and selection mode
 * - Advanced sorting (12+ options)
 ...
 */
export function AdvancedCharacterManager({ 
  projectId, 
  selectedCharacterId, 
  onClearSelection,
  tabConfig 
}: AdvancedCharacterManagerProps) {
  // All state management from original CharacterManager
  const [searchQuery, setSearchQuery] = useState('');
  const [sortBy, setSortBy] = useState<SortOption>('alphabetical');
  const [viewMode, setViewMode] = useState<ViewMode>(() => {
    const saved = localStorage.getItem(`${tabConfig.name}ViewMode`);
    return (saved as ViewMode) || tabConfig.ui.defaultViewMode;
  });
  ...
  // All queries from original CharacterManager
  ...
  useEffect(() => {
    localStorage.setItem(`${tabConfig.name}ViewMode`, viewMode);
  }, [viewMode, tabConfig.name]);

  // All mutations from original CharacterManager
  ...
  // EXACT completion calculation from original CharacterManager
  ...
  // EXACT sorting logic from original CharacterManager
  const sortCharacters = (chars: Character[]): Character[] => {
    switch (sortBy) {
      ...
      case 'by-completion':
        return [...chars].sort((a, b) => getCompletionPercentage(b) - getCompletionPercentage(a));
  ...
  // All handler functions from original CharacterManager
  ...
  // Premium Character Card for Grid View (customized with tab config)
    ...
    <Card className={`group cursor-pointer transition-all duration-300 hover:shadow-2xl hover:scale-[1.03] border overflow-hidden relative ${
      ...
      <CardContent className="p-0 relative">
            ...
            <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-accent/10 to-muted/30">
              <div className="text-center">
                <div className="w-20 h-20 mx-auto mb-3 bg-accent/20 rounded-full flex items-center justify-center">
                  <tabConfig.icon className="h-10 w-10 text-accent/60" />
                </div>
        ...
        <div className="p-6 space-y-4 relative">
          ...
          {tabConfig.features.hasCompletionTracking && (
  ...
  // Premium List View Item (customized with tab config)
        ...
        <div className="flex items-center gap-5 relative">
          ...
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-accent/10 via-muted/20 to-accent/15 flex items-center justify-center flex-shrink-0 border border-accent/20 shadow-md group-hover:shadow-lg transition-shadow duration-200">
            {character.imageUrl ? (
              ...
              <div className="w-10 h-10 bg-accent/20 rounded-full flex items-center justify-center">
                <tabConfig.icon className="h-6 w-6 text-accent/70" />
              </div>
            )}
          </div>
          ...
          {!isSelectionMode && (
            <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all duration-200">
              ...
              {tabConfig.features.hasPortraits && (
                <Button size="sm" variant="ghost" className="h-10 w-10 p-0 hover:bg-accent/10 hover:text-accent transition-colors rounded-xl"
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
          ...
          <div className="flex gap-3">
            ...
            >
              <div className="flex items-center">
                ...
                <span className="font-semibold tracking-wide">Create {tabConfig.displayName}</span>
              </div>
            </Button>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between gap-4 p-4 bg-muted/30 rounded-lg border border-border/50">
          ...
          <div className="relative flex-1 max-w-md">
            ...
            <Input
              placeholder={`Search ${tabConfig.displayName.toLowerCase()} by name, role, or race...`}
              value={searchQuery}
          ...
          <div className="flex items-center gap-2">
            ...
            {tabConfig.features.hasBulkOperations && (
              <Button
            ...
            {isSelectionMode && tabConfig.features.hasBulkOperations && (
              <>
                <Button
                  variant="outline"
                  size="sm"
                ...
                {tabConfig.features.hasBulkDelete && (
                  <Button
                    variant="destructive"
                    size="sm"
            ...
            {tabConfig.features.hasSorting && (
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-56">
                  {tabConfig.features.sortOptions.map((option) => (
                    <DropdownMenuItem key={option.key} onClick={() => setSortBy(option.key as SortOption)}>
                      {option.label}
                    </DropdownMenuItem>
                  ))}
                </DropdownMenuContent>
            ...
            {/* View Mode Toggle */}
            {tabConfig.features.hasViewModes && (
      ...
      {isLoading ? (
        <div className="flex items-center justify-center py-12">
          <div className="text-center space-y-2">
            ...
            <p className="text-muted-foreground">Loading {tabConfig.displayName.toLowerCase()}...</p>
          </div>
        </div>
      ) : filteredCharacters.length === 0 ? (
        <div className="text-center py-12 space-y-4">
          <tabConfig.icon className="h-16 w-16 text-muted-foreground/50 mx-auto" />
          <div>
            <h3 className="text-lg font-semibold mb-2">
              {characters.length === 0 ? `No ${tabConfig.displayName.toLowerCase()} yet` : `No ${tabConfig.displayName.toLowerCase()} match your search`}
            </h3>
            <p className="text-muted-foreground max-w-md mx-auto">
              {characters.length === 0 
                ? `Create your first ${tabConfig.displayName.toLowerCase()} to start building your story world.`
            ...
            <div className="flex gap-3 justify-center pt-4">
              ...
              >
                <div className="flex items-center">
                  ...
                  <span className="font-semibold tracking-wide">Create First {tabConfig.displayName}</span>
                </div>
              </Button>
            </div>
          )}
      ...
      {/* Modals - Only render if features are enabled */}
      {tabConfig.features.hasAIGeneration && (
      ...
      {tabConfig.features.hasPortraits && portraitCharacter && (
      ...
      <CharacterCreationLaunch
        ...
        onOpenTemplates={tabConfig.features.hasTemplates ? () => {
          setIsCreationLaunchOpen(false);
          setIsTemplateModalOpen(true);
        } : undefined}
        onOpenAIGeneration={tabConfig.features.hasAIGeneration ? () => {
          setIsCreationLaunchOpen(false);
          setIsGenerationModalOpen(true);
        } : undefined}
        onOpenDocumentUpload={tabConfig.features.hasDocumentUpload ? () => {
          setIsCreationLaunchOpen(false);
          setIsDocumentUploadOpen(true);
        } : undefined}
      />

      {tabConfig.features.hasDocumentUpload && (
      ...
      {tabConfig.features.hasTemplates && (
        <CharacterTemplates
