...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Universal AI Enhancement Routes
  const entityTypes = ['location', 'faction', 'item', 'organization', 'magic-system', 'timeline-event', 'language', 'culture', 'prophecy', 'theme'];
  
  entityTypes.forEach(entityType => {
    // Universal AI enhancement endpoint
    app.post(`/api/${entityType}s/:id/enhance`, async (req, res) => {
      try {
        const { id } = req.params;
        const { selectedCategories } = req.body;
        const entityData = req.body;
        
        // Import the universal AI generation service
        const { universalAIGeneration } = await import('./services/universalAIGeneration');
        
        const enhancedData = await universalAIGeneration.enhanceEntity(
          entityType,
          entityData,
          selectedCategories
        );
        ...
        console.error(`Error enhancing ${entityType}:`, error);
        res.status(500).json({ 
          error: `Failed to enhance ${entityType}`, 
          details: error.message 
        });
      }
    });
    ...
    app.post(`/api/${entityType}s/:id/generate-image`, async (req, res) => {
      try {
        const { id } = req.params;
        const { prompt, engine, imageType } = req.body;
        
        // Import the universal AI generation service
        const { universalAIGeneration } = await import('./services/universalAIGeneration');
        
        const imageUrl = await universalAIGeneration.generateEntityImage(
          entityType,
          req.body,
          prompt,
          imageType
        ...
        res.status(500).json({ 
          error: `Failed to generate image for ${entityType}`, 
          details: error.message 
        });
      }
    });

    // Universal field enhancement endpoint
    app.post(`/api/${entityType}s/:id/enhance-field`, async (req, res) => {
      try {
        const { id } = req.params;
        const { fieldKey, fieldLabel, currentValue } = req.body;
        
        // Import the universal AI generation service
        const { universalAIGeneration } = await import('./services/universalAIGeneration');
        
        const enhancedContent = await universalAIGeneration.generateEntityField(
          entityType,
          fieldKey,
          fieldLabel,
          req.body,
        ...
        res.json({ [fieldKey]: enhancedContent });
      } catch (error: any) {
        console.error(`Error enhancing field for ${entityType}:`, error);
        res.status(500).json({ 
          error: `Failed to enhance field`, 
          details: error.message 
        });
