...
Return ONLY a JSON object with appropriate fields for a faction.
...
Make it creative, original, and compelling!`;
  ...
  try {
    ...
    const factionData = JSON.parse(jsonMatch[0]);
    
    return factionData as Faction;
  } catch (error) {
    console.error('Faction generation error:', error);
    throw new Error(`Failed to generate faction: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
