...
export async function generateContextualItem(
  ...
  const prompt = `You are a creative writing assistant specializing in world-building. Create a detailed, original item for the project "${projectName}".
...
Return ONLY a JSON object with appropriate fields for a item.
...
Make it creative, original, and compelling!`;
  ...
  try {
    ...
    throw new Error(`Failed to generate item: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
