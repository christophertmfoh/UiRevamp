...
export async function generateContextualCreature(
  ...
  const prompt = `You are a creative writing assistant specializing in world-building. Create a detailed, original creature for the project "${projectName}".
...
Return ONLY a JSON object with appropriate fields for a creature.
...
Make it creative, original, and compelling!`;
  ...
  try {
    ...
    throw new Error(`Failed to generate creature: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
