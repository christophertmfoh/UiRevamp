...
export async function generateContextualOrganization(
  ...
  options: OrganizationGenerationOptions = {}
  ...
  const prompt = `You are a creative writing assistant specializing in world-building. Create a detailed, original organization for the project "${projectName}".
...
Return ONLY a JSON object with appropriate fields for a organization.
...
Make it creative, original, and compelling!`;
  ...
  try {
    ...
    const organizationData = JSON.parse(jsonMatch[0]);
    
    return organizationData as Organization;
  } catch (error) {
    console.error('Organization generation error:', error);
    throw new Error(`Failed to generate organization: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
