...
export async function generateContextualTimelineEvent(
  ...
  options: TimelineEventGenerationOptions = {}
  ...
  const prompt = `You are a creative writing assistant specializing in world-building. Create a detailed, original timelineevent for the project "${projectName}".
...
Return ONLY a JSON object with appropriate fields for a timelineevent.
...
Make it creative, original, and compelling!`;
  ...
  try {
    ...
    const timelineeventData = JSON.parse(jsonMatch[0]);
    
    return timelineeventData as TimelineEvent;
  } catch (error) {
    console.error('TimelineEvent generation error:', error);
    throw new Error(`Failed to generate timelineevent: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
