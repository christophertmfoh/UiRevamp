...
interface CreatureUnifiedViewProps {
  projectId: string;
  creature: Creature;
  onBack: () => void;
...
export function CreatureUnifiedView({ 
  projectId,
  creature, 
  onBack, 
  onDelete 
}: CreatureUnifiedViewProps) {
  ...
  const saveMutation = useMutation({
    mutationFn: async (data: Creature) => {
      return await apiRequest('PUT', `/api/creatures/${creature.id}`, data);
    },
    onSuccess: () => {
  ...
  const handleSave = () => {
    ...
    saveMutation.mutate(processedData as Creature);
  };

  const handleCancel = () => {
    setFormData(creature); // Reset form data
    setIsEditing(false);
  };

  const processDataForSave = (data: Creature) => {
    ...
    if (creature.portraits) {
      dataToSave.portraits = creature.portraits;
    }
  ...
  const handleImageGenerated = (imageUrl: string) => {
    // Update creature with new image, preserving portraits
    const updatedData = { 
      ...formData, 
      imageUrl,
      portraits: creature.portraits || [] // Preserve existing portraits
    };
  ...
  const handleImageUploaded = (imageUrl: string) => {
    // Update creature with uploaded image, preserving portraits
    const updatedData = { 
      ...formData, 
      imageUrl,
      portraits: creature.portraits || [] // Preserve existing portraits
          ...
          {!isEditing ? (
            ...
            <>
              ...
              >
                ...
                {saveMutation.isPending ? 'Saving...' : 'Save Creature'}
              </Button>
          ...
          <div className="flex items-start gap-6">
            ...
            <div className="flex-1">
              <h1 className="font-title text-3xl mb-2">
                {formData.name || 'Unnamed Creature'}
              </h1>
