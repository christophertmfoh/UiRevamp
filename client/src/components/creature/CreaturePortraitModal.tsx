...
interface CreaturePortraitModalProps {
  creature: Creature;
  isOpen: boolean;
  onClose: () => void;
...
export function CreaturePortraitModal({
  creature,
  isOpen,
  onClose,
  onImageGenerated,
  onImageUploaded
}: CreaturePortraitModalProps) {
  ...
  const [portraitGallery, setPortraitGallery] = useState<Array<{id: string, url: string, isMain: boolean}>>(() => {
    // Initialize from creature.portraits if available
    const existingPortraits = creature.portraits || [];
    return Array.isArray(existingPortraits) ? existingPortraits : [];
  });
  const [selectedMainImage, setSelectedMainImage] = useState<string>('');

  // Generate comprehensive AI prompt from all physical creature data
  const generateCreaturePrompt = () => {
    const parts = [];
    
    // Basic info
    if (creature.name) parts.push(`Creature named ${creature.name}`);
    if (creature.age) parts.push(`${creature.age} years old`);
    if (creature.race) parts.push(creature.race);
    if (creature.ethnicity) parts.push(creature.ethnicity);
    
    // Physical build and body
    if (creature.build) parts.push(`${creature.build} build`);
    if (creature.bodyType) parts.push(`${creature.bodyType} body type`);
    if (creature.height) parts.push(`${creature.height} tall`);
    if (creature.posture) parts.push(`${creature.posture} posture`);
    
    // Facial features
    if (creature.facialFeatures) parts.push(creature.facialFeatures);
    if (creature.eyes || creature.eyeColor) {
      const eyeDesc = [creature.eyeColor, creature.eyes].filter(Boolean).join(' ');
      parts.push(`${eyeDesc} eyes`);
    }
    if (creature.complexion) parts.push(`${creature.complexion} complexion`);
    if (creature.skin || creature.skinTone) {
      const skinDesc = [creature.skinTone, creature.skin].filter(Boolean).join(' ');
      parts.push(`${skinDesc} skin`);
    }
    
    // Hair
    if (creature.hair || creature.hairColor || creature.hairStyle) {
      const hairDesc = [creature.hairColor, creature.hairStyle, creature.hair].filter(Boolean).join(' ');
      parts.push(`${hairDesc} hair`);
    }
    if (creature.facialHair) parts.push(creature.facialHair);
    
    // Clothing and accessories
    if (creature.attire) parts.push(`wearing ${creature.attire}`);
    if (creature.clothingStyle) parts.push(`${creature.clothingStyle} clothing style`);
    if (creature.accessories) parts.push(`accessories: ${creature.accessories}`);
    
    // Distinguishing features
    if (creature.scars) parts.push(`scars: ${creature.scars}`);
    if (creature.tattoos) parts.push(`tattoos: ${creature.tattoos}`);
    if (creature.piercings) parts.push(`piercings: ${creature.piercings}`);
    if (creature.birthmarks) parts.push(`birthmarks: ${creature.birthmarks}`);
    if (creature.distinguishingMarks) parts.push(`distinguishing marks: ${creature.distinguishingMarks}`);
    
    // General physical description
    if (creature.physicalDescription) parts.push(creature.physicalDescription);
  ...
  const handleGenerateImage = async () => {
    ...
    try {
      const creaturePrompt = generateCreaturePrompt();
      
      console.log('Generating image with creature prompt:', creaturePrompt);
      console.log('Style prompt:', stylePrompt);
      console.log('Using AI engine:', aiEngine);

      const response = await fetch('/api/creatures/generate-image', {
        method: 'POST',
        headers: {
  ...
  const handleImageGenerated = (imageUrl: string) => {
    ...
    // Update creature with new image and save portraits array
    onImageGenerated?.(imageUrl);
    
    // If this is the main image, also update the creature's imageUrl
    if (newPortrait.isMain) {
      updateCreatureImageUrl(imageUrl, updated);
    } else {
      // Just save the portraits array
      savePortraitsToCreature(updated);
    }
  };
  ...
  const handleSetMainImage = (imageId: string) => {
    ...
    if (mainImage) {
      // Update the creature's main image in the parent components
      ...
      // Also update the creature's imageUrl field directly
      updateCreatureImageUrl(mainImage.url, updated);
    } else {
  ...
  const handleDeletePortrait = (imageId: string) => {
    ...
    if (deletedImage?.isMain && updated.length > 0) {
      ...
      // Update the creature's imageUrl with the new main image
      updateCreatureImageUrl(finalUpdated[0].url, finalUpdated);
    } else if (deletedImage?.isMain && updated.length === 0) {
      // No images left, clear the creature image
      onImageGenerated?.('');
      updateCreatureImageUrl('', finalUpdated);
    } else {
      // Save the updated portraits array
      savePortraitsToCreature(finalUpdated);
    }
    
    setPortraitGallery(finalUpdated);
  };

  // Helper function to save portraits to creature
  const savePortraitsToCreature = async (updatedPortraits: Array<{id: string, url: string, isMain: boolean}>) => {
    try {
      ...
      if (!response.ok) {
        console.error('Failed to save portraits to creature');
      }
    } catch (error) {
  ...
  // Helper function to update creature's main imageUrl
  const updateCreatureImageUrl = async (imageUrl: string, updatedPortraits: Array<{id: string, url: string, isMain: boolean}>) => {
    try {
      const response = await fetch(`/api/creatures/${creature.id}`, {
        method: 'PUT',
        headers: {
      ...
      if (!response.ok) {
        console.error('Failed to update creature imageUrl');
      } else {
        // Invalidate cache to refresh the creature list
        queryClient.invalidateQueries({ queryKey: ['/api/projects', creature.projectId, 'creatures'] });
      }
    } catch (error) {
      console.error('Error updating creature imageUrl:', error);
    }
  };

  const handleModalClose = () => {
    // Save the current gallery state to the creature
    savePortraitsToCreature(portraitGallery);
    
    // If there's a main image in the gallery, make sure it's saved to the creature
    const mainImage = portraitGallery.find(img => img.isMain);
    if (mainImage && mainImage.url !== creature.imageUrl) {
      onImageGenerated?.(mainImage.url);
    }
    onClose();
  };
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto creative-card" aria-describedby="portrait-description">
        <DialogHeader>
          <DialogTitle className="font-title text-2xl">
            Manage Portraits for {creature.name || 'Creature'}
          </DialogTitle>
          <div id="portrait-description" className="sr-only">
            Generate AI portraits or upload images for your creature. You can create multiple portraits and set one as the main creature image.
          </div>
        </DialogHeader>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              ...
              <div className="space-y-4">
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    {/* Creature Prompt Preview */}
                    <div>
                      <Label>Creature Description (Auto-generated)</Label>
                      <Textarea
                        value={generateCreaturePrompt() || 'Add creature details in the Creature Editor to auto-generate description...'}
                        readOnly
                        className="creative-input text-sm text-muted-foreground"
                        rows={3}
                      />
                      <p className="text-xs text-muted-foreground mt-1">
                        This description compiles all physical information from your creature profile
                      </p>
                    </div>
          ...
          <TabsContent value="train" className="space-y-6">
            <Card>
              ...
              <CardContent className="space-y-4">
                <p className="text-sm text-muted-foreground">
                  Select 6-10 images from the gallery to create a consistent visual model for this creature.
                </p>
          ...
          <DialogContent className="max-w-4xl max-h-[90vh] p-2" aria-describedby="portrait-preview-description">
            <DialogHeader>
              ...
              <div id="portrait-preview-description" className="sr-only">
                View enlarged creature portrait and set as main image if desired.
              </div>
            </DialogHeader>
            <div className="flex items-center justify-center max-h-[80vh]">
              <img 
                src={selectedMainImage} 
                alt="Creature portrait enlarged"
