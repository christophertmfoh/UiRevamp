...
interface LanguagePortraitModalProps {
  language: Language;
  isOpen: boolean;
  onClose: () => void;
...
export function LanguagePortraitModal({
  language,
  isOpen,
  onClose,
  onImageGenerated,
  onImageUploaded
}: LanguagePortraitModalProps) {
  ...
  const [portraitGallery, setPortraitGallery] = useState<Array<{id: string, url: string, isMain: boolean}>>(() => {
    // Initialize from language.portraits if available
    const existingPortraits = language.portraits || [];
    return Array.isArray(existingPortraits) ? existingPortraits : [];
  });
  const [selectedMainImage, setSelectedMainImage] = useState<string>('');

  // Generate comprehensive AI prompt from all physical language data
  const generateLanguagePrompt = () => {
    const parts = [];
    
    // Basic info
    if (language.name) parts.push(`Language named ${language.name}`);
    if (language.age) parts.push(`${language.age} years old`);
    if (language.race) parts.push(language.race);
    if (language.ethnicity) parts.push(language.ethnicity);
    
    // Physical build and body
    if (language.build) parts.push(`${language.build} build`);
    if (language.bodyType) parts.push(`${language.bodyType} body type`);
    if (language.height) parts.push(`${language.height} tall`);
    if (language.posture) parts.push(`${language.posture} posture`);
    
    // Facial features
    if (language.facialFeatures) parts.push(language.facialFeatures);
    if (language.eyes || language.eyeColor) {
      const eyeDesc = [language.eyeColor, language.eyes].filter(Boolean).join(' ');
      parts.push(`${eyeDesc} eyes`);
    }
    if (language.complexion) parts.push(`${language.complexion} complexion`);
    if (language.skin || language.skinTone) {
      const skinDesc = [language.skinTone, language.skin].filter(Boolean).join(' ');
      parts.push(`${skinDesc} skin`);
    }
    
    // Hair
    if (language.hair || language.hairColor || language.hairStyle) {
      const hairDesc = [language.hairColor, language.hairStyle, language.hair].filter(Boolean).join(' ');
      parts.push(`${hairDesc} hair`);
    }
    if (language.facialHair) parts.push(language.facialHair);
    
    // Clothing and accessories
    if (language.attire) parts.push(`wearing ${language.attire}`);
    if (language.clothingStyle) parts.push(`${language.clothingStyle} clothing style`);
    if (language.accessories) parts.push(`accessories: ${language.accessories}`);
    
    // Distinguishing features
    if (language.scars) parts.push(`scars: ${language.scars}`);
    if (language.tattoos) parts.push(`tattoos: ${language.tattoos}`);
    if (language.piercings) parts.push(`piercings: ${language.piercings}`);
    if (language.birthmarks) parts.push(`birthmarks: ${language.birthmarks}`);
    if (language.distinguishingMarks) parts.push(`distinguishing marks: ${language.distinguishingMarks}`);
    
    // General physical description
    if (language.physicalDescription) parts.push(language.physicalDescription);
  ...
  const handleGenerateImage = async () => {
    ...
    try {
      const languagePrompt = generateLanguagePrompt();
      
      console.log('Generating image with language prompt:', languagePrompt);
      console.log('Style prompt:', stylePrompt);
      console.log('Using AI engine:', aiEngine);

      const response = await fetch('/api/languages/generate-image', {
        method: 'POST',
        headers: {
  ...
  const handleImageGenerated = (imageUrl: string) => {
    ...
    // Update language with new image and save portraits array
    onImageGenerated?.(imageUrl);
    
    // If this is the main image, also update the language's imageUrl
    if (newPortrait.isMain) {
      updateLanguageImageUrl(imageUrl, updated);
    } else {
      // Just save the portraits array
      savePortraitsToLanguage(updated);
    }
  };
  ...
  const handleSetMainImage = (imageId: string) => {
    ...
    if (mainImage) {
      // Update the language's main image in the parent components
      ...
      // Also update the language's imageUrl field directly
      updateLanguageImageUrl(mainImage.url, updated);
    } else {
  ...
  const handleDeletePortrait = (imageId: string) => {
    ...
    if (deletedImage?.isMain && updated.length > 0) {
      ...
      // Update the language's imageUrl with the new main image
      updateLanguageImageUrl(finalUpdated[0].url, finalUpdated);
    } else if (deletedImage?.isMain && updated.length === 0) {
      // No images left, clear the language image
      onImageGenerated?.('');
      updateLanguageImageUrl('', finalUpdated);
    } else {
      // Save the updated portraits array
      savePortraitsToLanguage(finalUpdated);
    }
    
    setPortraitGallery(finalUpdated);
  };

  // Helper function to save portraits to language
  const savePortraitsToLanguage = async (updatedPortraits: Array<{id: string, url: string, isMain: boolean}>) => {
    try {
      ...
      if (!response.ok) {
        console.error('Failed to save portraits to language');
      }
    } catch (error) {
  ...
  // Helper function to update language's main imageUrl
  const updateLanguageImageUrl = async (imageUrl: string, updatedPortraits: Array<{id: string, url: string, isMain: boolean}>) => {
    try {
      const response = await fetch(`/api/languages/${language.id}`, {
        method: 'PUT',
        headers: {
      ...
      if (!response.ok) {
        console.error('Failed to update language imageUrl');
      } else {
        // Invalidate cache to refresh the language list
        queryClient.invalidateQueries({ queryKey: ['/api/projects', language.projectId, 'languages'] });
      }
    } catch (error) {
      console.error('Error updating language imageUrl:', error);
    }
  };

  const handleModalClose = () => {
    // Save the current gallery state to the language
    savePortraitsToLanguage(portraitGallery);
    
    // If there's a main image in the gallery, make sure it's saved to the language
    const mainImage = portraitGallery.find(img => img.isMain);
    if (mainImage && mainImage.url !== language.imageUrl) {
      onImageGenerated?.(mainImage.url);
    }
    onClose();
  };
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto creative-card" aria-describedby="portrait-description">
        <DialogHeader>
          <DialogTitle className="font-title text-2xl">
            Manage Portraits for {language.name || 'Language'}
          </DialogTitle>
          <div id="portrait-description" className="sr-only">
            Generate AI portraits or upload images for your language. You can create multiple portraits and set one as the main language image.
          </div>
        </DialogHeader>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              ...
              <div className="space-y-4">
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div>
                      <Label>Language Description (Auto-generated)</Label>
                      <Textarea
                        value={generateLanguagePrompt() || 'Add language details in the Language Editor to auto-generate description...'}
                        readOnly
                        className="creative-input text-sm text-muted-foreground"
                        rows={3}
                      />
                      <p className="text-xs text-muted-foreground mt-1">
                        This description compiles all physical information from your language profile
                      </p>
                    </div>
          ...
          <TabsContent value="train" className="space-y-6">
            <Card>
              ...
              <CardContent className="space-y-4">
                <p className="text-sm text-muted-foreground">
                  Select 6-10 images from the gallery to create a consistent visual model for this language.
                </p>
          ...
          <DialogContent className="max-w-4xl max-h-[90vh] p-2" aria-describedby="portrait-preview-description">
            <DialogHeader>
              ...
              <div id="portrait-preview-description" className="sr-only">
                View enlarged language portrait and set as main image if desired.
              </div>
            </DialogHeader>
            <div className="flex items-center justify-center max-h-[80vh]">
              <img 
                src={selectedMainImage} 
                alt="Language portrait enlarged"
