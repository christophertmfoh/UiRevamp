...
interface LanguageUnifiedViewProps {
  projectId: string;
  language: Language;
  onBack: () => void;
...
export function LanguageUnifiedView({ 
  projectId,
  language, 
  onBack, 
  onDelete 
}: LanguageUnifiedViewProps) {
  ...
  const saveMutation = useMutation({
    mutationFn: async (data: Language) => {
      return await apiRequest('PUT', `/api/languages/${language.id}`, data);
    },
    onSuccess: () => {
  ...
  const handleSave = () => {
    ...
    saveMutation.mutate(processedData as Language);
  };

  const handleCancel = () => {
    setFormData(language); // Reset form data
    setIsEditing(false);
  };

  const processDataForSave = (data: Language) => {
    ...
    if (language.portraits) {
      dataToSave.portraits = language.portraits;
    }
  ...
  const handleImageGenerated = (imageUrl: string) => {
    // Update language with new image, preserving portraits
    const updatedData = { 
      ...formData, 
      imageUrl,
      portraits: language.portraits || [] // Preserve existing portraits
    };
  ...
  const handleImageUploaded = (imageUrl: string) => {
    // Update language with uploaded image, preserving portraits
    const updatedData = { 
      ...formData, 
      imageUrl,
      portraits: language.portraits || [] // Preserve existing portraits
          ...
          <div className="flex items-start gap-6">
            ...
            <div className="flex-1">
              <h1 className="font-title text-3xl mb-2">
                {formData.name || 'Unnamed Language'}
              </h1>
