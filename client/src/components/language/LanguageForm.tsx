...
export function LanguageForm({ projectId, onCancel, language }: LanguageFormProps) {
  ...
  const createMutation = useMutation({
    mutationFn: (data: any) => apiRequest('POST', `/api/projects/${projectId}/languages`, data),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: (data: any) => apiRequest('PUT', `/api/languages/${language?.id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/projects', projectId, 'languages'] });
      onCancel();
    },
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const processedData = {
      id: language?.id || Date.now().toString(),
      projectId,
      name: formData.name,
            ...
            <Tabs defaultValue="basic" className="w-full">
              ...
              <TabsContent value="basic" className="space-y-4">
                ...
                <div className="border-2 border-dashed border-muted-foreground/25 rounded-lg p-8 text-center">
                  ...
                  <h3 className="text-lg font-semibold mb-2">Language Image</h3>
