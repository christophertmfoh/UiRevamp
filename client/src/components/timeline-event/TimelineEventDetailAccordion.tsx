...
interface TimelineEventDetailAccordionProps {
  projectId: string;
...
export function TimelineEventDetailAccordion({ 
  timelineevent, 
  onBack, 
  onEdit, 
  onDelete 
}: TimelineEventDetailAccordionProps) {
  ...
  const sectionHasContent = (sectionId: string) => {
    ...
    return section.fields.some(field => {
      const value = (timelineevent as any)[field.key];
      if (field.type === 'array') {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button onClick={() => onEdit(timelineevent)} className="interactive-warm gap-2">
          ...
          <div className="flex items-start gap-6">
            ...
            <div className="flex-1">
              <h1 className="font-title text-3xl mb-2">
                {timelineevent.name || 'Unnamed TimelineEvent'}
              </h1>
              
              {timelineevent.title && (
                <p className="text-lg text-muted-foreground mb-3 italic">"{timelineevent.title}"</p>
              )}
