...
interface TimelineEventUnifiedViewProps {
  projectId: string;
...
export function TimelineEventUnifiedView({ 
  projectId,
  timelineevent, 
  onBack, 
  onDelete 
}: TimelineEventUnifiedViewProps) {
  ...
  const saveMutation = useMutation({
    mutationFn: async (data: TimelineEvent) => {
      return await apiRequest('PUT', `/api/timelineevents/${timelineevent.id}`, data);
    },
    onSuccess: () => {
    ...
    onError: (error) => {
      console.error('Failed to save timelineevent:', error);
    }
  });

  const handleSave = () => {
    const processedData = processDataForSave(formData);
    saveMutation.mutate(processedData as TimelineEvent);
  };

  const handleCancel = () => {
    setFormData(timelineevent); // Reset form data
    setIsEditing(false);
  };

  const processDataForSave = (data: TimelineEvent) => {
    ...
    if (timelineevent.portraits) {
      dataToSave.portraits = timelineevent.portraits;
    }
  ...
  const handleImageGenerated = (imageUrl: string) => {
    // Update timelineevent with new image, preserving portraits
    const updatedData = { 
      ...formData, 
      imageUrl,
      portraits: timelineevent.portraits || [] // Preserve existing portraits
    };
  ...
  const handleImageUploaded = (imageUrl: string) => {
    // Update timelineevent with uploaded image, preserving portraits
    const updatedData = { 
      ...formData, 
      imageUrl,
      portraits: timelineevent.portraits || [] // Preserve existing portraits
        ...
        <div className="flex gap-2">
          {!isEditing ? (
            <>
              ...
              <Button 
                variant="destructive" 
                onClick={() => onDelete(timelineevent)}
                className="gap-2"
              >
                <X className="h-4 w-4" />
                Delete
            ...
            <>
              ...
              >
                ...
                {saveMutation.isPending ? 'Saving...' : 'Save TimelineEvent'}
              </Button>
      ...
      <Card className="creative-card">
        <CardContent className="p-6">
          <div className="flex items-start gap-6">
            ...
            {/* TimelineEvent Basic Info */}
            <div className="flex-1">
              <h1 className="font-title text-3xl mb-2">
                {formData.name || 'Unnamed TimelineEvent'}
              </h1>
