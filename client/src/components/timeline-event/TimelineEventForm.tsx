...
export function TimelineEventForm({ projectId, onCancel, timelineevent }: TimelineEventFormProps) {
  ...
  const createMutation = useMutation({
    mutationFn: (data: any) => apiRequest('POST', `/api/projects/${projectId}/timelineevents`, data),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: (data: any) => apiRequest('PUT', `/api/timelineevents/${timelineevent?.id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/projects', projectId, 'timelineevents'] });
      onCancel();
    },
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const processedData = {
      id: timelineevent?.id || Date.now().toString(),
      projectId,
      name: formData.name,
            ...
            <Tabs defaultValue="basic" className="w-full">
              ...
              <TabsContent value="basic" className="space-y-4">
                ...
                <div className="border-2 border-dashed border-muted-foreground/25 rounded-lg p-8 text-center">
                  ...
                  <h3 className="text-lg font-semibold mb-2">TimelineEvent Image</h3>
