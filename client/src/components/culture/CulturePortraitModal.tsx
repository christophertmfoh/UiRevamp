...
interface CulturePortraitModalProps {
  culture: Culture;
  isOpen: boolean;
  onClose: () => void;
...
export function CulturePortraitModal({
  culture,
  isOpen,
  onClose,
  onImageGenerated,
  onImageUploaded
}: CulturePortraitModalProps) {
  ...
  const [portraitGallery, setPortraitGallery] = useState<Array<{id: string, url: string, isMain: boolean}>>(() => {
    // Initialize from culture.portraits if available
    const existingPortraits = culture.portraits || [];
    return Array.isArray(existingPortraits) ? existingPortraits : [];
  });
  const [selectedMainImage, setSelectedMainImage] = useState<string>('');

  // Generate comprehensive AI prompt from all physical culture data
  const generateCulturePrompt = () => {
    const parts = [];
    
    // Basic info
    if (culture.name) parts.push(`Culture named ${culture.name}`);
    if (culture.age) parts.push(`${culture.age} years old`);
    if (culture.race) parts.push(culture.race);
    if (culture.ethnicity) parts.push(culture.ethnicity);
    
    // Physical build and body
    if (culture.build) parts.push(`${culture.build} build`);
    if (culture.bodyType) parts.push(`${culture.bodyType} body type`);
    if (culture.height) parts.push(`${culture.height} tall`);
    if (culture.posture) parts.push(`${culture.posture} posture`);
    
    // Facial features
    if (culture.facialFeatures) parts.push(culture.facialFeatures);
    if (culture.eyes || culture.eyeColor) {
      const eyeDesc = [culture.eyeColor, culture.eyes].filter(Boolean).join(' ');
      parts.push(`${eyeDesc} eyes`);
    }
    if (culture.complexion) parts.push(`${culture.complexion} complexion`);
    if (culture.skin || culture.skinTone) {
      const skinDesc = [culture.skinTone, culture.skin].filter(Boolean).join(' ');
      parts.push(`${skinDesc} skin`);
    }
    
    // Hair
    if (culture.hair || culture.hairColor || culture.hairStyle) {
      const hairDesc = [culture.hairColor, culture.hairStyle, culture.hair].filter(Boolean).join(' ');
      parts.push(`${hairDesc} hair`);
    }
    if (culture.facialHair) parts.push(culture.facialHair);
    
    // Clothing and accessories
    if (culture.attire) parts.push(`wearing ${culture.attire}`);
    if (culture.clothingStyle) parts.push(`${culture.clothingStyle} clothing style`);
    if (culture.accessories) parts.push(`accessories: ${culture.accessories}`);
    
    // Distinguishing features
    if (culture.scars) parts.push(`scars: ${culture.scars}`);
    if (culture.tattoos) parts.push(`tattoos: ${culture.tattoos}`);
    if (culture.piercings) parts.push(`piercings: ${culture.piercings}`);
    if (culture.birthmarks) parts.push(`birthmarks: ${culture.birthmarks}`);
    if (culture.distinguishingMarks) parts.push(`distinguishing marks: ${culture.distinguishingMarks}`);
    
    // General physical description
    if (culture.physicalDescription) parts.push(culture.physicalDescription);
  ...
  const handleGenerateImage = async () => {
    ...
    try {
      const culturePrompt = generateCulturePrompt();
      
      console.log('Generating image with culture prompt:', culturePrompt);
      console.log('Style prompt:', stylePrompt);
      console.log('Using AI engine:', aiEngine);

      const response = await fetch('/api/cultures/generate-image', {
        method: 'POST',
        headers: {
  ...
  const handleImageGenerated = (imageUrl: string) => {
    ...
    // Update culture with new image and save portraits array
    onImageGenerated?.(imageUrl);
    
    // If this is the main image, also update the culture's imageUrl
    if (newPortrait.isMain) {
      updateCultureImageUrl(imageUrl, updated);
    } else {
      // Just save the portraits array
      savePortraitsToCulture(updated);
    }
  };
  ...
  const handleSetMainImage = (imageId: string) => {
    ...
    if (mainImage) {
      // Update the culture's main image in the parent components
      ...
      // Also update the culture's imageUrl field directly
      updateCultureImageUrl(mainImage.url, updated);
    } else {
  ...
  const handleDeletePortrait = (imageId: string) => {
    ...
    if (deletedImage?.isMain && updated.length > 0) {
      ...
      // Update the culture's imageUrl with the new main image
      updateCultureImageUrl(finalUpdated[0].url, finalUpdated);
    } else if (deletedImage?.isMain && updated.length === 0) {
      // No images left, clear the culture image
      onImageGenerated?.('');
      updateCultureImageUrl('', finalUpdated);
    } else {
      // Save the updated portraits array
      savePortraitsToCulture(finalUpdated);
    }
    
    setPortraitGallery(finalUpdated);
  };

  // Helper function to save portraits to culture
  const savePortraitsToCulture = async (updatedPortraits: Array<{id: string, url: string, isMain: boolean}>) => {
    try {
      ...
      if (!response.ok) {
        console.error('Failed to save portraits to culture');
      }
    } catch (error) {
  ...
  // Helper function to update culture's main imageUrl
  const updateCultureImageUrl = async (imageUrl: string, updatedPortraits: Array<{id: string, url: string, isMain: boolean}>) => {
    try {
      const response = await fetch(`/api/cultures/${culture.id}`, {
        method: 'PUT',
        headers: {
      ...
      if (!response.ok) {
        console.error('Failed to update culture imageUrl');
      } else {
        // Invalidate cache to refresh the culture list
        queryClient.invalidateQueries({ queryKey: ['/api/projects', culture.projectId, 'cultures'] });
      }
    } catch (error) {
      console.error('Error updating culture imageUrl:', error);
    }
  };

  const handleModalClose = () => {
    // Save the current gallery state to the culture
    savePortraitsToCulture(portraitGallery);
    
    // If there's a main image in the gallery, make sure it's saved to the culture
    const mainImage = portraitGallery.find(img => img.isMain);
    if (mainImage && mainImage.url !== culture.imageUrl) {
      onImageGenerated?.(mainImage.url);
    }
    onClose();
  };
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto creative-card" aria-describedby="portrait-description">
        <DialogHeader>
          <DialogTitle className="font-title text-2xl">
            Manage Portraits for {culture.name || 'Culture'}
          </DialogTitle>
          <div id="portrait-description" className="sr-only">
            Generate AI portraits or upload images for your culture. You can create multiple portraits and set one as the main culture image.
          </div>
        </DialogHeader>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              ...
              <div className="space-y-4">
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    {/* Culture Prompt Preview */}
                    <div>
                      <Label>Culture Description (Auto-generated)</Label>
                      <Textarea
                        value={generateCulturePrompt() || 'Add culture details in the Culture Editor to auto-generate description...'}
                        readOnly
                        className="creative-input text-sm text-muted-foreground"
                        rows={3}
                      />
                      <p className="text-xs text-muted-foreground mt-1">
                        This description compiles all physical information from your culture profile
                      </p>
                    </div>
          ...
          <TabsContent value="train" className="space-y-6">
            <Card>
              ...
              <CardContent className="space-y-4">
                <p className="text-sm text-muted-foreground">
                  Select 6-10 images from the gallery to create a consistent visual model for this culture.
                </p>
          ...
          <DialogContent className="max-w-4xl max-h-[90vh] p-2" aria-describedby="portrait-preview-description">
            <DialogHeader>
              ...
              <div id="portrait-preview-description" className="sr-only">
                View enlarged culture portrait and set as main image if desired.
              </div>
            </DialogHeader>
            <div className="flex items-center justify-center max-h-[80vh]">
              <img 
                src={selectedMainImage} 
                alt="Culture portrait enlarged"
