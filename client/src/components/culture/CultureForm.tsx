...
export function CultureForm({ projectId, onCancel, culture }: CultureFormProps) {
  ...
  const createMutation = useMutation({
    mutationFn: (data: any) => apiRequest('POST', `/api/projects/${projectId}/cultures`, data),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: (data: any) => apiRequest('PUT', `/api/cultures/${culture?.id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/projects', projectId, 'cultures'] });
      onCancel();
    },
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const processedData = {
      id: culture?.id || Date.now().toString(),
      projectId,
      name: formData.name,
            ...
            <Tabs defaultValue="basic" className="w-full">
              ...
              <TabsContent value="basic" className="space-y-4">
                ...
                <div className="border-2 border-dashed border-muted-foreground/25 rounded-lg p-8 text-center">
                  ...
                  <h3 className="text-lg font-semibold mb-2">Culture Image</h3>
