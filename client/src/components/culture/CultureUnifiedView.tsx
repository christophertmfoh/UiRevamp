...
interface CultureUnifiedViewProps {
  projectId: string;
  culture: Culture;
  onBack: () => void;
...
export function CultureUnifiedView({ 
  projectId,
  culture, 
  onBack, 
  onDelete 
}: CultureUnifiedViewProps) {
  ...
  const saveMutation = useMutation({
    mutationFn: async (data: Culture) => {
      return await apiRequest('PUT', `/api/cultures/${culture.id}`, data);
    },
    onSuccess: () => {
    ...
    onError: (error) => {
      console.error('Failed to save culture:', error);
    }
  });

  const handleSave = () => {
    const processedData = processDataForSave(formData);
    saveMutation.mutate(processedData as Culture);
  };

  const handleCancel = () => {
    setFormData(culture); // Reset form data
    setIsEditing(false);
  };

  const processDataForSave = (data: Culture) => {
    ...
    if (culture.portraits) {
      dataToSave.portraits = culture.portraits;
    }
  ...
  const handleImageGenerated = (imageUrl: string) => {
    // Update culture with new image, preserving portraits
    const updatedData = { 
      ...formData, 
      imageUrl,
      portraits: culture.portraits || [] // Preserve existing portraits
    };
  ...
  const handleImageUploaded = (imageUrl: string) => {
    // Update culture with uploaded image, preserving portraits
    const updatedData = { 
      ...formData, 
      imageUrl,
      portraits: culture.portraits || [] // Preserve existing portraits
          ...
          {!isEditing ? (
            ...
            <>
              ...
              >
                ...
                {saveMutation.isPending ? 'Saving...' : 'Save Culture'}
              </Button>
