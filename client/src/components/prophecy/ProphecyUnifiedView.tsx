...
interface ProphecyUnifiedViewProps {
  projectId: string;
  prophecy: Prophecy;
  onBack: () => void;
...
export function ProphecyUnifiedView({ 
  projectId,
  prophecy, 
  onBack, 
  onDelete 
}: ProphecyUnifiedViewProps) {
  const [isEditing, setIsEditing] = useState(false);
  const [formData, setFormData] = useState(prophecy);
  ...
  const saveMutation = useMutation({
    mutationFn: async (data: Prophecy) => {
      return await apiRequest('PUT', `/api/prophecys/${prophecy.id}`, data);
    },
    onSuccess: () => {
    ...
    onError: (error) => {
      console.error('Failed to save prophecy:', error);
    }
  });

  const handleSave = () => {
    const processedData = processDataForSave(formData);
    saveMutation.mutate(processedData as Prophecy);
  };

  const handleCancel = () => {
    setFormData(prophecy); // Reset form data
    setIsEditing(false);
  };

  const processDataForSave = (data: Prophecy) => {
    ...
    if (prophecy.portraits) {
      dataToSave.portraits = prophecy.portraits;
    }
  ...
  const handleImageGenerated = (imageUrl: string) => {
    // Update prophecy with new image, preserving portraits
    const updatedData = { 
      ...formData, 
      imageUrl,
      portraits: prophecy.portraits || [] // Preserve existing portraits
    };
  ...
  const handleImageUploaded = (imageUrl: string) => {
    // Update prophecy with uploaded image, preserving portraits
    const updatedData = { 
      ...formData, 
      imageUrl,
      portraits: prophecy.portraits || [] // Preserve existing portraits
          ...
          {!isEditing ? (
            ...
            <>
              ...
              >
                ...
                {saveMutation.isPending ? 'Saving...' : 'Save Prophecy'}
              </Button>
          ...
          <div className="flex items-start gap-6">
            ...
            <div className="flex-1">
              <h1 className="font-title text-3xl mb-2">
                {formData.name || 'Unnamed Prophecy'}
              </h1>
