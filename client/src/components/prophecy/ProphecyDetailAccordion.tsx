...
interface ProphecyDetailAccordionProps {
  projectId: string;
  prophecy: Prophecy;
  onBack: () => void;
...
export function ProphecyDetailAccordion({ 
  prophecy, 
  onBack, 
  onEdit, 
  onDelete 
}: ProphecyDetailAccordionProps) {
  ...
  const sectionHasContent = (sectionId: string) => {
    ...
    return section.fields.some(field => {
      const value = (prophecy as any)[field.key];
      if (field.type === 'array') {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button onClick={() => onEdit(prophecy)} className="interactive-warm gap-2">
          ...
          <div className="flex items-start gap-6">
            ...
            <div className="flex-1">
              <h1 className="font-title text-3xl mb-2">
                {prophecy.name || 'Unnamed Prophecy'}
              </h1>
              
              {prophecy.title && (
                <p className="text-lg text-muted-foreground mb-3 italic">"{prophecy.title}"</p>
              )}
