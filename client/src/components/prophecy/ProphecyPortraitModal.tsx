...
interface ProphecyPortraitModalProps {
  prophecy: Prophecy;
  isOpen: boolean;
  onClose: () => void;
...
export function ProphecyPortraitModal({
  prophecy,
  isOpen,
  onClose,
  onImageGenerated,
  onImageUploaded
}: ProphecyPortraitModalProps) {
  ...
  const [portraitGallery, setPortraitGallery] = useState<Array<{id: string, url: string, isMain: boolean}>>(() => {
    // Initialize from prophecy.portraits if available
    const existingPortraits = prophecy.portraits || [];
    return Array.isArray(existingPortraits) ? existingPortraits : [];
  });
  const [selectedMainImage, setSelectedMainImage] = useState<string>('');

  // Generate comprehensive AI prompt from all physical prophecy data
  const generateProphecyPrompt = () => {
    const parts = [];
    
    // Basic info
    if (prophecy.name) parts.push(`Prophecy named ${prophecy.name}`);
    if (prophecy.age) parts.push(`${prophecy.age} years old`);
    if (prophecy.race) parts.push(prophecy.race);
    if (prophecy.ethnicity) parts.push(prophecy.ethnicity);
    
    // Physical build and body
    if (prophecy.build) parts.push(`${prophecy.build} build`);
    if (prophecy.bodyType) parts.push(`${prophecy.bodyType} body type`);
    if (prophecy.height) parts.push(`${prophecy.height} tall`);
    if (prophecy.posture) parts.push(`${prophecy.posture} posture`);
    
    // Facial features
    if (prophecy.facialFeatures) parts.push(prophecy.facialFeatures);
    if (prophecy.eyes || prophecy.eyeColor) {
      const eyeDesc = [prophecy.eyeColor, prophecy.eyes].filter(Boolean).join(' ');
      parts.push(`${eyeDesc} eyes`);
    }
    if (prophecy.complexion) parts.push(`${prophecy.complexion} complexion`);
    if (prophecy.skin || prophecy.skinTone) {
      const skinDesc = [prophecy.skinTone, prophecy.skin].filter(Boolean).join(' ');
      parts.push(`${skinDesc} skin`);
    }
    
    // Hair
    if (prophecy.hair || prophecy.hairColor || prophecy.hairStyle) {
      const hairDesc = [prophecy.hairColor, prophecy.hairStyle, prophecy.hair].filter(Boolean).join(' ');
      parts.push(`${hairDesc} hair`);
    }
    if (prophecy.facialHair) parts.push(prophecy.facialHair);
    
    // Clothing and accessories
    if (prophecy.attire) parts.push(`wearing ${prophecy.attire}`);
    if (prophecy.clothingStyle) parts.push(`${prophecy.clothingStyle} clothing style`);
    if (prophecy.accessories) parts.push(`accessories: ${prophecy.accessories}`);
    
    // Distinguishing features
    if (prophecy.scars) parts.push(`scars: ${prophecy.scars}`);
    if (prophecy.tattoos) parts.push(`tattoos: ${prophecy.tattoos}`);
    if (prophecy.piercings) parts.push(`piercings: ${prophecy.piercings}`);
    if (prophecy.birthmarks) parts.push(`birthmarks: ${prophecy.birthmarks}`);
    if (prophecy.distinguishingMarks) parts.push(`distinguishing marks: ${prophecy.distinguishingMarks}`);
    
    // General physical description
    if (prophecy.physicalDescription) parts.push(prophecy.physicalDescription);
  ...
  const handleGenerateImage = async () => {
    ...
    try {
      const prophecyPrompt = generateProphecyPrompt();
      
      console.log('Generating image with prophecy prompt:', prophecyPrompt);
      console.log('Style prompt:', stylePrompt);
      console.log('Using AI engine:', aiEngine);

      const response = await fetch('/api/prophecys/generate-image', {
        method: 'POST',
        headers: {
  ...
  const handleImageGenerated = (imageUrl: string) => {
    ...
    // Update prophecy with new image and save portraits array
    onImageGenerated?.(imageUrl);
    
    // If this is the main image, also update the prophecy's imageUrl
    if (newPortrait.isMain) {
      updateProphecyImageUrl(imageUrl, updated);
    } else {
      // Just save the portraits array
      savePortraitsToProphecy(updated);
    }
  };
  ...
  const handleSetMainImage = (imageId: string) => {
    ...
    if (mainImage) {
      // Update the prophecy's main image in the parent components
      ...
      // Also update the prophecy's imageUrl field directly
      updateProphecyImageUrl(mainImage.url, updated);
    } else {
  ...
  const handleDeletePortrait = (imageId: string) => {
    ...
    if (deletedImage?.isMain && updated.length > 0) {
      ...
      // Update the prophecy's imageUrl with the new main image
      updateProphecyImageUrl(finalUpdated[0].url, finalUpdated);
    } else if (deletedImage?.isMain && updated.length === 0) {
      // No images left, clear the prophecy image
      onImageGenerated?.('');
      updateProphecyImageUrl('', finalUpdated);
    } else {
      // Save the updated portraits array
      savePortraitsToProphecy(finalUpdated);
    }
    
    setPortraitGallery(finalUpdated);
  };

  // Helper function to save portraits to prophecy
  const savePortraitsToProphecy = async (updatedPortraits: Array<{id: string, url: string, isMain: boolean}>) => {
    try {
      ...
      if (!response.ok) {
        console.error('Failed to save portraits to prophecy');
      }
    } catch (error) {
  ...
  // Helper function to update prophecy's main imageUrl
  const updateProphecyImageUrl = async (imageUrl: string, updatedPortraits: Array<{id: string, url: string, isMain: boolean}>) => {
    try {
      const response = await fetch(`/api/prophecys/${prophecy.id}`, {
        method: 'PUT',
        headers: {
      ...
      if (!response.ok) {
        console.error('Failed to update prophecy imageUrl');
      } else {
        // Invalidate cache to refresh the prophecy list
        queryClient.invalidateQueries({ queryKey: ['/api/projects', prophecy.projectId, 'prophecys'] });
      }
    } catch (error) {
      console.error('Error updating prophecy imageUrl:', error);
    }
  };

  const handleModalClose = () => {
    // Save the current gallery state to the prophecy
    savePortraitsToProphecy(portraitGallery);
    
    // If there's a main image in the gallery, make sure it's saved to the prophecy
    const mainImage = portraitGallery.find(img => img.isMain);
    if (mainImage && mainImage.url !== prophecy.imageUrl) {
      onImageGenerated?.(mainImage.url);
    }
    onClose();
  };
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto creative-card" aria-describedby="portrait-description">
        <DialogHeader>
          <DialogTitle className="font-title text-2xl">
            Manage Portraits for {prophecy.name || 'Prophecy'}
          </DialogTitle>
          <div id="portrait-description" className="sr-only">
            Generate AI portraits or upload images for your prophecy. You can create multiple portraits and set one as the main prophecy image.
          </div>
        </DialogHeader>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              ...
              <div className="space-y-4">
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    {/* Prophecy Prompt Preview */}
                    <div>
                      <Label>Prophecy Description (Auto-generated)</Label>
                      <Textarea
                        value={generateProphecyPrompt() || 'Add prophecy details in the Prophecy Editor to auto-generate description...'}
                        readOnly
                        className="creative-input text-sm text-muted-foreground"
                        rows={3}
                      />
                      <p className="text-xs text-muted-foreground mt-1">
                        This description compiles all physical information from your prophecy profile
                      </p>
                    </div>
          ...
          <TabsContent value="train" className="space-y-6">
            <Card>
              ...
              <CardContent className="space-y-4">
                <p className="text-sm text-muted-foreground">
                  Select 6-10 images from the gallery to create a consistent visual model for this prophecy.
                </p>
          ...
          <DialogContent className="max-w-4xl max-h-[90vh] p-2" aria-describedby="portrait-preview-description">
            <DialogHeader>
              ...
              <div id="portrait-preview-description" className="sr-only">
                View enlarged prophecy portrait and set as main image if desired.
              </div>
            </DialogHeader>
            <div className="flex items-center justify-center max-h-[80vh]">
              <img 
                src={selectedMainImage} 
                alt="Prophecy portrait enlarged"
