...
interface MagicSystemUnifiedViewProps {
  projectId: string;
...
export function MagicSystemUnifiedView({ 
  projectId,
  magicsystem, 
  onBack, 
  onDelete 
}: MagicSystemUnifiedViewProps) {
  ...
  const saveMutation = useMutation({
    mutationFn: async (data: MagicSystem) => {
      return await apiRequest('PUT', `/api/magicsystems/${magicsystem.id}`, data);
    },
    onSuccess: () => {
    ...
    onError: (error) => {
      console.error('Failed to save magicsystem:', error);
    }
  });

  const handleSave = () => {
    const processedData = processDataForSave(formData);
    saveMutation.mutate(processedData as MagicSystem);
  };

  const handleCancel = () => {
    setFormData(magicsystem); // Reset form data
    setIsEditing(false);
  };

  const processDataForSave = (data: MagicSystem) => {
    ...
    if (magicsystem.portraits) {
      dataToSave.portraits = magicsystem.portraits;
    }
  ...
  const handleImageGenerated = (imageUrl: string) => {
    // Update magicsystem with new image, preserving portraits
    const updatedData = { 
      ...formData, 
      imageUrl,
      portraits: magicsystem.portraits || [] // Preserve existing portraits
    };
  ...
  const handleImageUploaded = (imageUrl: string) => {
    // Update magicsystem with uploaded image, preserving portraits
    const updatedData = { 
      ...formData, 
      imageUrl,
      portraits: magicsystem.portraits || [] // Preserve existing portraits
          ...
          {!isEditing ? (
            ...
            <>
              ...
              >
                ...
                {saveMutation.isPending ? 'Saving...' : 'Save MagicSystem'}
              </Button>
          ...
          <div className="flex items-start gap-6">
            ...
            <div className="flex-1">
              <h1 className="font-title text-3xl mb-2">
                {formData.name || 'Unnamed MagicSystem'}
              </h1>
