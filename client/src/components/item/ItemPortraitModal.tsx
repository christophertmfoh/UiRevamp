...
interface ItemPortraitModalProps {
  item: Item;
  isOpen: boolean;
  onClose: () => void;
...
export function ItemPortraitModal({
  item,
  isOpen,
  onClose,
  onImageGenerated,
  onImageUploaded
}: ItemPortraitModalProps) {
  ...
  const [portraitGallery, setPortraitGallery] = useState<Array<{id: string, url: string, isMain: boolean}>>(() => {
    // Initialize from item.portraits if available
    const existingPortraits = item.portraits || [];
    return Array.isArray(existingPortraits) ? existingPortraits : [];
  });
  const [selectedMainImage, setSelectedMainImage] = useState<string>('');

  // Generate comprehensive AI prompt from all physical item data
  const generateItemPrompt = () => {
    const parts = [];
    
    // Basic info
    if (item.name) parts.push(`Item named ${item.name}`);
    if (item.age) parts.push(`${item.age} years old`);
    if (item.race) parts.push(item.race);
    if (item.ethnicity) parts.push(item.ethnicity);
    
    // Physical build and body
    if (item.build) parts.push(`${item.build} build`);
    if (item.bodyType) parts.push(`${item.bodyType} body type`);
    if (item.height) parts.push(`${item.height} tall`);
    if (item.posture) parts.push(`${item.posture} posture`);
    
    // Facial features
    if (item.facialFeatures) parts.push(item.facialFeatures);
    if (item.eyes || item.eyeColor) {
      const eyeDesc = [item.eyeColor, item.eyes].filter(Boolean).join(' ');
      parts.push(`${eyeDesc} eyes`);
    }
    if (item.complexion) parts.push(`${item.complexion} complexion`);
    if (item.skin || item.skinTone) {
      const skinDesc = [item.skinTone, item.skin].filter(Boolean).join(' ');
      parts.push(`${skinDesc} skin`);
    }
    
    // Hair
    if (item.hair || item.hairColor || item.hairStyle) {
      const hairDesc = [item.hairColor, item.hairStyle, item.hair].filter(Boolean).join(' ');
      parts.push(`${hairDesc} hair`);
    }
    if (item.facialHair) parts.push(item.facialHair);
    
    // Clothing and accessories
    if (item.attire) parts.push(`wearing ${item.attire}`);
    if (item.clothingStyle) parts.push(`${item.clothingStyle} clothing style`);
    if (item.accessories) parts.push(`accessories: ${item.accessories}`);
    
    // Distinguishing features
    if (item.scars) parts.push(`scars: ${item.scars}`);
    if (item.tattoos) parts.push(`tattoos: ${item.tattoos}`);
    if (item.piercings) parts.push(`piercings: ${item.piercings}`);
    if (item.birthmarks) parts.push(`birthmarks: ${item.birthmarks}`);
    if (item.distinguishingMarks) parts.push(`distinguishing marks: ${item.distinguishingMarks}`);
    
    // General physical description
    if (item.physicalDescription) parts.push(item.physicalDescription);
  ...
  const handleGenerateImage = async () => {
    ...
    try {
      ...
      console.log('Generating image with item prompt:', itemPrompt);
  ...
  const handleImageGenerated = (imageUrl: string) => {
    ...
    // Update item with new image and save portraits array
    onImageGenerated?.(imageUrl);
    
    // If this is the main image, also update the item's imageUrl
    if (newPortrait.isMain) {
      updateItemImageUrl(imageUrl, updated);
    } else {
      // Just save the portraits array
      savePortraitsToItem(updated);
    }
  };
  ...
  const handleSetMainImage = (imageId: string) => {
    ...
    if (mainImage) {
      // Update the item's main image in the parent components
      ...
      // Also update the item's imageUrl field directly
      updateItemImageUrl(mainImage.url, updated);
    } else {
  ...
  const handleDeletePortrait = (imageId: string) => {
    ...
    if (deletedImage?.isMain && updated.length > 0) {
      ...
      // Update the item's imageUrl with the new main image
      updateItemImageUrl(finalUpdated[0].url, finalUpdated);
    } else if (deletedImage?.isMain && updated.length === 0) {
      // No images left, clear the item image
      onImageGenerated?.('');
      updateItemImageUrl('', finalUpdated);
    } else {
      // Save the updated portraits array
      savePortraitsToItem(finalUpdated);
    }
    
    setPortraitGallery(finalUpdated);
  };

  // Helper function to save portraits to item
  const savePortraitsToItem = async (updatedPortraits: Array<{id: string, url: string, isMain: boolean}>) => {
    try {
      ...
      if (!response.ok) {
        console.error('Failed to save portraits to item');
      }
    } catch (error) {
  ...
  // Helper function to update item's main imageUrl
  const updateItemImageUrl = async (imageUrl: string, updatedPortraits: Array<{id: string, url: string, isMain: boolean}>) => {
    try {
      const response = await fetch(`/api/items/${item.id}`, {
        method: 'PUT',
        headers: {
      ...
      if (!response.ok) {
        console.error('Failed to update item imageUrl');
      } else {
        // Invalidate cache to refresh the item list
        queryClient.invalidateQueries({ queryKey: ['/api/projects', item.projectId, 'items'] });
      }
    } catch (error) {
      console.error('Error updating item imageUrl:', error);
    }
  };

  const handleModalClose = () => {
    // Save the current gallery state to the item
    savePortraitsToItem(portraitGallery);
    
    // If there's a main image in the gallery, make sure it's saved to the item
    const mainImage = portraitGallery.find(img => img.isMain);
    if (mainImage && mainImage.url !== item.imageUrl) {
      onImageGenerated?.(mainImage.url);
    }
    onClose();
  };
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto creative-card" aria-describedby="portrait-description">
        <DialogHeader>
          <DialogTitle className="font-title text-2xl">
            Manage Portraits for {item.name || 'Item'}
          </DialogTitle>
          <div id="portrait-description" className="sr-only">
            Generate AI portraits or upload images for your item. You can create multiple portraits and set one as the main item image.
          </div>
        </DialogHeader>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              ...
              <div className="space-y-4">
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    {/* Item Prompt Preview */}
                    <div>
                      <Label>Item Description (Auto-generated)</Label>
                      <Textarea
                        value={generateItemPrompt() || 'Add item details in the Item Editor to auto-generate description...'}
                        readOnly
                        className="creative-input text-sm text-muted-foreground"
                        rows={3}
                      />
                      <p className="text-xs text-muted-foreground mt-1">
                        This description compiles all physical information from your item profile
                      </p>
                    </div>
          ...
          <TabsContent value="train" className="space-y-6">
            <Card>
              ...
              <CardContent className="space-y-4">
                <p className="text-sm text-muted-foreground">
                  Select 6-10 images from the gallery to create a consistent visual model for this item.
                </p>
          ...
          <DialogContent className="max-w-4xl max-h-[90vh] p-2" aria-describedby="portrait-preview-description">
            <DialogHeader>
              ...
              <div id="portrait-preview-description" className="sr-only">
                View enlarged item portrait and set as main image if desired.
              </div>
            </DialogHeader>
            <div className="flex items-center justify-center max-h-[80vh]">
              <img 
                src={selectedMainImage} 
                alt="Item portrait enlarged"
