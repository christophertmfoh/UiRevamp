...
interface ItemUnifiedViewProps {
  projectId: string;
  item: Item;
  onBack: () => void;
...
export function ItemUnifiedView({ 
  projectId,
  item, 
  onBack, 
  onDelete 
}: ItemUnifiedViewProps) {
  ...
  const saveMutation = useMutation({
    mutationFn: async (data: Item) => {
      return await apiRequest('PUT', `/api/items/${item.id}`, data);
    },
    onSuccess: () => {
  ...
  const handleSave = () => {
    ...
    saveMutation.mutate(processedData as Item);
  };
  ...
  const processDataForSave = (data: Item) => {
    ...
    if (item.portraits) {
      dataToSave.portraits = item.portraits;
    }
  ...
  const handleImageGenerated = (imageUrl: string) => {
    // Update item with new image, preserving portraits
    const updatedData = { 
      ...formData, 
      imageUrl,
      portraits: item.portraits || [] // Preserve existing portraits
    };
  ...
  const handleImageUploaded = (imageUrl: string) => {
    // Update item with uploaded image, preserving portraits
    const updatedData = { 
      ...formData, 
      imageUrl,
      portraits: item.portraits || [] // Preserve existing portraits
