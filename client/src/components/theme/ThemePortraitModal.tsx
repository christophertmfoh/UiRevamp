...
interface ThemePortraitModalProps {
  theme: Theme;
  isOpen: boolean;
  onClose: () => void;
...
export function ThemePortraitModal({
  theme,
  isOpen,
  onClose,
  onImageGenerated,
  onImageUploaded
}: ThemePortraitModalProps) {
  ...
  const [portraitGallery, setPortraitGallery] = useState<Array<{id: string, url: string, isMain: boolean}>>(() => {
    // Initialize from theme.portraits if available
    const existingPortraits = theme.portraits || [];
    return Array.isArray(existingPortraits) ? existingPortraits : [];
  });
  const [selectedMainImage, setSelectedMainImage] = useState<string>('');

  // Generate comprehensive AI prompt from all physical theme data
  const generateThemePrompt = () => {
    const parts = [];
    
    // Basic info
    if (theme.name) parts.push(`Theme named ${theme.name}`);
    if (theme.age) parts.push(`${theme.age} years old`);
    if (theme.race) parts.push(theme.race);
    if (theme.ethnicity) parts.push(theme.ethnicity);
    
    // Physical build and body
    if (theme.build) parts.push(`${theme.build} build`);
    if (theme.bodyType) parts.push(`${theme.bodyType} body type`);
    if (theme.height) parts.push(`${theme.height} tall`);
    if (theme.posture) parts.push(`${theme.posture} posture`);
    
    // Facial features
    if (theme.facialFeatures) parts.push(theme.facialFeatures);
    if (theme.eyes || theme.eyeColor) {
      const eyeDesc = [theme.eyeColor, theme.eyes].filter(Boolean).join(' ');
      parts.push(`${eyeDesc} eyes`);
    }
    if (theme.complexion) parts.push(`${theme.complexion} complexion`);
    if (theme.skin || theme.skinTone) {
      const skinDesc = [theme.skinTone, theme.skin].filter(Boolean).join(' ');
      parts.push(`${skinDesc} skin`);
    }
    
    // Hair
    if (theme.hair || theme.hairColor || theme.hairStyle) {
      const hairDesc = [theme.hairColor, theme.hairStyle, theme.hair].filter(Boolean).join(' ');
      parts.push(`${hairDesc} hair`);
    }
    if (theme.facialHair) parts.push(theme.facialHair);
    
    // Clothing and accessories
    if (theme.attire) parts.push(`wearing ${theme.attire}`);
    if (theme.clothingStyle) parts.push(`${theme.clothingStyle} clothing style`);
    if (theme.accessories) parts.push(`accessories: ${theme.accessories}`);
    
    // Distinguishing features
    if (theme.scars) parts.push(`scars: ${theme.scars}`);
    if (theme.tattoos) parts.push(`tattoos: ${theme.tattoos}`);
    if (theme.piercings) parts.push(`piercings: ${theme.piercings}`);
    if (theme.birthmarks) parts.push(`birthmarks: ${theme.birthmarks}`);
    if (theme.distinguishingMarks) parts.push(`distinguishing marks: ${theme.distinguishingMarks}`);
    
    // General physical description
    if (theme.physicalDescription) parts.push(theme.physicalDescription);
  ...
  const handleGenerateImage = async () => {
    ...
    try {
      ...
      console.log('Generating image with theme prompt:', themePrompt);
      console.log('Style prompt:', stylePrompt);
      console.log('Using AI engine:', aiEngine);

      const response = await fetch('/api/themes/generate-image', {
        method: 'POST',
        headers: {
  ...
  const handleImageGenerated = (imageUrl: string) => {
    ...
    // Update theme with new image and save portraits array
    onImageGenerated?.(imageUrl);
    
    // If this is the main image, also update the theme's imageUrl
    if (newPortrait.isMain) {
      updateThemeImageUrl(imageUrl, updated);
    } else {
      // Just save the portraits array
      savePortraitsToTheme(updated);
    }
  };
  ...
  const handleSetMainImage = (imageId: string) => {
    ...
    if (mainImage) {
      // Update the theme's main image in the parent components
      ...
      // Also update the theme's imageUrl field directly
      updateThemeImageUrl(mainImage.url, updated);
    } else {
  ...
  const handleDeletePortrait = (imageId: string) => {
    ...
    if (deletedImage?.isMain && updated.length > 0) {
      ...
      // Update the theme's imageUrl with the new main image
      updateThemeImageUrl(finalUpdated[0].url, finalUpdated);
    } else if (deletedImage?.isMain && updated.length === 0) {
      // No images left, clear the theme image
      onImageGenerated?.('');
      updateThemeImageUrl('', finalUpdated);
    } else {
      // Save the updated portraits array
      savePortraitsToTheme(finalUpdated);
    }
    
    setPortraitGallery(finalUpdated);
  };

  // Helper function to save portraits to theme
  const savePortraitsToTheme = async (updatedPortraits: Array<{id: string, url: string, isMain: boolean}>) => {
    try {
      ...
      if (!response.ok) {
        console.error('Failed to save portraits to theme');
      }
    } catch (error) {
  ...
  // Helper function to update theme's main imageUrl
  const updateThemeImageUrl = async (imageUrl: string, updatedPortraits: Array<{id: string, url: string, isMain: boolean}>) => {
    try {
      const response = await fetch(`/api/themes/${theme.id}`, {
        method: 'PUT',
        headers: {
      ...
      if (!response.ok) {
        console.error('Failed to update theme imageUrl');
      } else {
        // Invalidate cache to refresh the theme list
        queryClient.invalidateQueries({ queryKey: ['/api/projects', theme.projectId, 'themes'] });
      }
    } catch (error) {
      console.error('Error updating theme imageUrl:', error);
    }
  };

  const handleModalClose = () => {
    // Save the current gallery state to the theme
    savePortraitsToTheme(portraitGallery);
    
    // If there's a main image in the gallery, make sure it's saved to the theme
    const mainImage = portraitGallery.find(img => img.isMain);
    if (mainImage && mainImage.url !== theme.imageUrl) {
      onImageGenerated?.(mainImage.url);
    }
    onClose();
  };
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto creative-card" aria-describedby="portrait-description">
        <DialogHeader>
          <DialogTitle className="font-title text-2xl">
            Manage Portraits for {theme.name || 'Theme'}
          </DialogTitle>
          <div id="portrait-description" className="sr-only">
            Generate AI portraits or upload images for your theme. You can create multiple portraits and set one as the main theme image.
          </div>
        </DialogHeader>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              ...
              <div className="space-y-4">
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div>
                      <Label>Theme Description (Auto-generated)</Label>
                      <Textarea
                        value={generateThemePrompt() || 'Add theme details in the Theme Editor to auto-generate description...'}
                        readOnly
                        className="creative-input text-sm text-muted-foreground"
                        rows={3}
                      />
                      <p className="text-xs text-muted-foreground mt-1">
                        This description compiles all physical information from your theme profile
                      </p>
                    </div>
          ...
          <TabsContent value="train" className="space-y-6">
            <Card>
              ...
              <CardContent className="space-y-4">
                <p className="text-sm text-muted-foreground">
                  Select 6-10 images from the gallery to create a consistent visual model for this theme.
                </p>
          ...
          <DialogContent className="max-w-4xl max-h-[90vh] p-2" aria-describedby="portrait-preview-description">
            <DialogHeader>
              ...
              <div id="portrait-preview-description" className="sr-only">
                View enlarged theme portrait and set as main image if desired.
              </div>
            </DialogHeader>
            <div className="flex items-center justify-center max-h-[80vh]">
              <img 
                src={selectedMainImage} 
                alt="Theme portrait enlarged"
